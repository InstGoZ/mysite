...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={'class':'form-control','placeholder':'请输入用户名和邮箱'}
        )
    )
...
class RegForm(forms.Form):
    username = forms.CharField(
        ...
        min_length=3,
        widget=forms.TextInput(
            attrs={'class':'form-control','placeholder':'请输入3-20长度的用户名'}
        )
    )
    ...
    # 执行is_valid()时会调用clean()方法, 下划线再加字段名是对字段的分别验证
